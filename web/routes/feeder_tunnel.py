...
def feeder_tunnel_proxy(feeder_id: str, subpath: str = ""):
    ...
    # Return expected shape instead of 404 so drawUpintheair() can safely read .upintheair.length.
    if status == 404 and path_only in ("/upintheair.json", "/data/upintheair.json"):
        return Response(b'{"upintheair":[]}', status=200, headers={"Content-Type": "application/json; charset=utf-8"})
