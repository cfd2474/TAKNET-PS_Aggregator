...
- **Map still shows dashboard / stats “no url”**  
  ...
  - If the feeder’s `feeder_id` uses dashes and the aggregator URL uses underscores (or vice versa), the aggregator tries alternate forms for **host lookup** and for **proxy routing** (so the tunnel connection is found even when register id differs only by `_` vs `-`). Prefer matching register `feeder_id` to the aggregator Connect URL when possible.
